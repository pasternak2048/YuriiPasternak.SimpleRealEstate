...
{
    var toCodifier = GetSafeString(firstSheet.Cells[row, 4].Text);
    ...
    if (toType == TerritorialObjectTypeEnum.City
        || toType == TerritorialObjectTypeEnum.SmallTown
        || toType == TerritorialObjectTypeEnum.Village)
    {
        ...
        Console.WriteLine();
        count++;
        Console.Write($"ADDED: {count}:  ||  {to.Id}  ||  {to.Katottg}  ||  {region?.Name}  ||  {district?.Name}  ||  {community?.Name}  ||  {to.Name}  ||  {to.TypeId}");
